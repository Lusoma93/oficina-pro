...
async function movePdfToMega(downloadsPath, megaPath, clienteNombre, monto, fecha, requestStartTime) {
  ...
  while (Date.now() - startTime < 60000) {
    ...
    const pdfFiles = files
      ...
      .map(f => {
        ...
        return { name: f, path: filePath, mtime: stats.mtimeMs, birthtimeMs: stats.birthtimeMs };
      })
      .filter(f => {
        const fileTime = Math.max(f.mtime, f.birthtimeMs || 0);
        return fileTime >= minFileTime;
      })
      .sort((a, b) => Math.max(b.mtime, b.birthtimeMs) - Math.max(a.mtime, a.birthtimeMs));
      
    if (pdfFiles.length > 0) {
      mostRecentPdf = pdfFiles[0];
      break;
    }
